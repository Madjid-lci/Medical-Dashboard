...
const PatientsCharts: React.FC<PatientsChartsProps> = ({ patientsData }) => {
  ...
  const totalPatients = referralCounts.needReferral + referralCounts.noReferral;

  const referralPieData = {
    labels: [
      `Need Referral (${Math.round((referralCounts.needReferral / totalPatients) * 100)}%)`,
      `No Referral (${Math.round((referralCounts.noReferral / totalPatients) * 100)}%)`
    ],
    datasets: [
      {
